...
namespace TextAnalysis
{
    static class TextGeneratorTask
    {
        ...
        {
            int phraseLenght = 1;
            var phraseBeginningArr = phraseBeginning.Split(' ');
            var wordKeyFirst = phraseBeginning;
            var wordKeyNext = phraseBeginning;
            var subWordKeyNext = phraseBeginning;
            ...
            phraseBeginning = MakePhraseBeginning(wordKeyFirst, wordKeyNext, wordsCount,
                phraseLenght, nextWords, subWordKeyNext, phraseBeginning, phraseBeginningArr, keyArr);
        ...
        private static string MakePhraseBeginning(string wordKeyFirst, string wordKeyNext, int wordsCount,
            int phraseLenght, Dictionary<string, string> nextWords, string subWordKeyNext, string phraseBeginning,
            string[] phraseBeginningArr, string[] keyArr)
        {
            while (true)
            {
                if (wordsCount < 1)
                    break;
                if (((phraseLenght > 1) || (phraseBeginningArr.Length > 1))
