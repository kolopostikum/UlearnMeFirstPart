	...
	public static class PathFinderTask
	{
		...
		public static int[] FindBestCheckpointsOrder(Point[] checkpoints)
		{
            if (checkpoints.Length > 1)
            {
				...
				var bestOrder = result[result.Count - 1];
				return bestOrder;
			}
			if (checkpoints.Length == 1)
				return new int[1];
        ...
        {
			var subRes = new int[permutation.Length];
        ...
        private static double CountLength(int position, Point[] checkpoints, int[] permutation)
        {
			...
			{
				length += PointExtensions.DistanceTo(checkpoints[permutation[i - 1]],
													 checkpoints[permutation[i]]);
			}
